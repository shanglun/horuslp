...
from horuslp_gurobi.core.utils import get_constraints_value, call_with_required_args


class GurobiProblemFeatures:
    def __init__(self, model: gr.Model):
        self.model = model

    def output_iis(self, output_path):
        if self.model.status != gr.GRB.INFEASIBLE:
            print('Model is not infeasible!')
            return
        self.model.computeIIS()
        self.model.write(output_path.rstrip('.ilp') + '.ilp')

    def output_lp(self, output_path):
        self.model.write(output_path.rstrip('.lp') + '.lp')

    def output_model_file(self, output_path):
...
class Problem:
    ...
    def __init__(self):
        ...
        self.constraint_objs = [c(self.model) for c in self.constraints]
    ...
    def flatten_constraints(self, constraint_objs):
        ...
        for constraint_obj in constraint_objs:
            ...
            dependent_objs = [c(self.model) for c in constraint_obj.dependent_constraints]
            flat_constraints.extend(self.flatten_constraints(dependent_objs))
        return flat_constraints

    def implement_constraint(self, constraint):
        """
        ...
        if constraint_spec is not None:
            ...
            self.model.addConstr(constraint_spec, name=constraint.name)
    ...
    def implement_constraints(self):
        ...
        for constraint in self.flattened_constraints:
            self.implement_constraint(constraint)

    def implement_objective(self):
        """
        Implement the objective for the model.
        :LPProblem prob: the Pulp LPProblem instance
        """
        obj = call_with_required_args(self.objective_obj.define, self.vars)
        pl_sense = gr.GRB.MAXIMIZE if self.sense == MAXIMIZE else gr.GRB.MINIMIZE
        self.model.setObjective(obj, pl_sense)
    ...
    def read_result_variables(self):
        ...
        for var_name, pl_var in self.vars.items():
            if isinstance(pl_var, OrderedDict):
                ...
                self.result_variables[var_name] = pl_var.x
    ...
    def print_results(self, find_infeasible=False, deep_infeasibility_search=False):
        ...
        """
        status = {
            gr.GRB.OPTIMAL: 'Optimal',
            gr.GRB.INFEASIBLE: 'Infeasible',
            gr.GRB.UNBOUNDED: 'Unbounded',
            gr.GRB.INF_OR_UNBD: 'Infeasible or Unbounded'
        }[self.model.status]
        print('%s: %s' % (self.name, status))
        if self.model.status == gr.GRB.OPTIMAL:
            self.print_optimal_results()
        elif self.model.status == gr.GRB.INFEASIBLE and find_infeasible:
